...
var unaryOperatorMap = {
  typeof: function _typeof(itprNode) {
    try {
      return _typeof2(itprNode.walk(itprNode.node.argument));
    } catch (err) {
...
var getMemberValueRef = function getMemberValueRef(node, itprNode) {
  ...
  var propRef = new ValueRef(obj, propName);
  return propRef;
};
export var es5 = {
  ...
  ForStatement: function ForStatement(itprNode) {
    ...
    var forScope = new Scope(ScopeType.BLOCK, itprNode.scope);
    var result;
  ...
  BlockStatement: function BlockStatement(itprNode) {
    var body = itprNode.node.body;
    var blockScope = new Scope(ScopeType.BLOCK, itprNode.scope);
    var statements = getHostingStatements(body);
    var result;
  ...
  FunctionDeclaration: function FunctionDeclaration(itprNode) {
    ...
    var fn = createFunction(itprNode);
  ...
  CallExpression: function CallExpression(itprNode) {
    ...
    return fn.call.apply(fn, [context].concat(_toConsumableArray(argsVal)));
  },
  ...
  NewExpression: function NewExpression(itprNode) {
    ...
    return _construct(Constructor, _toConsumableArray(argsValue));
  },
  ...
  WithStatement: function WithStatement(itprNode) {
    ...
    var withScope = new Scope(ScopeType.BLOCK, scope, scopeValue);
